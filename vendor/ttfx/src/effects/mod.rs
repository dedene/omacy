...
impl EffectCommand {
    ...
    /// Construct an effect from its registry name using struct/Clap defaults.
    pub fn from_name(name: &str) -> Option<Box<dyn Effect>> {
        let cli = Cli::try_parse_from(["ttfx", name]).ok()?;
        Some(cli.effect?.build_effect())
    }
