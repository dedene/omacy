...
final class OmacyRenderer: NSObject {
    ...
    private func applyUnpresentedGeometry() {
        ...
        if createSession(
            ...
            settings: lastSettings,
            art: lastArt,
            identity: transitions.replacementIdentity(settings: lastSettings, art: lastArt, cols: pc, rows: pr)
        ) {
            cols = pc
            rows = pr
            promotePendingFont()
    ...
    private func applyWaitingWork(_ result: OmacyStepResult) {
        ...
        if result.needs_begin_next != 0 {
            ...
            let candidateSettings = boundary.settings
            ...
            let plan = transitions.boundaryPlan(
                settings: candidateSettings, art: candidateArt, cols: nextCols, rows: nextRows
            )
            ...
            if plan.configuration.background != session.configuration.background {
            ...
            transitions.performBoundary(
                ...
                transition: { [weak self] candidate in
                    ...
                    case .ignoredUntilIdentityChanges, .failed(.rejected): return false
    ...
    private func createSession(
        ...
        if let session {
            preparation = session.replacement(
                configuration: transitions.engineConfiguration(settings: settings, art: art),
                cols: cols,
                rows: rows
            )
        } else {
    ...
    private func loadContent() {
        if let pinned = pinnedContent {
            ...
            let snapshot = OmacyStore.loadConfiguration()
